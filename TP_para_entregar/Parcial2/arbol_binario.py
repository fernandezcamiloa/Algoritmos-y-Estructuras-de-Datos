...
#parcial 2
    def inorden_nombre(self, name):
        def __inorden_nombre(root, name):
            if root is not None:
                __inorden_nombre(root.left, name)
                if root.other_values is name:
                    print(root.value)
                __inorden_nombre(root.right, name)

        __inorden_nombre(self.root, name)
    ...
    def contar_electrico(self):
        def __contar_electrico(root):
            ...
            if root is not None:
                if root.other_values is True:
                    count = 1
                count += __contar_electrico(root.left)
                count += __contar_electrico(root.right)
            return count
    ...
    def contar_acero(self):
        def __contar_acero(root):
            ...
            if root is not None:
                if root.other_values is True:
                    count = 1
                count += __contar_acero(root.left)
                count += __contar_acero(root.right)
            return count
