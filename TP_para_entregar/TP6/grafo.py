...
#Funciones ejercicio 14
    def existe_paso(self, origen, destino):
        resultado = False
        ver_origen = self.search_vertice(origen)
        if not ver_origen.visitado:
            ver_origen.visitado = True
            #print(vert_origen.info)
            adyacentes = ver_origen.adyacentes.get_inicio()
...
#Funciones ejercicio 15
    def contar_maravillas(self):
        ...
        while aux is not None:
            ...
            for p in pais:
                if p not in paises:
                    paises[p] = {'arq': False, 'nat': False}

                if aux.datos['tipo'] == 'Natural':
                    paises[p]['nat'] = True
            
                if aux.datos['tipo'] == 'Arquitectónica':
                    paises[p]['arq'] = True
            aux = aux.sig

        return paises

    def paises_mas_de_una_maravilla_del_mismo_tipo(self):
        paises_naturales = []
        paises_arquitectonicas = []
        aux = self.__inicio
        while aux is not None:
            pais = aux.datos['pais']
            ...
            for p in pais:
                if aux.datos['tipo'] == 'Natural':
                    paises_naturales.append(p)
            
                if aux.datos['tipo'] == 'Arquitectónica':
                    paises_arquitectonicas.append(p)
            aux = aux.sig
