...
class BinaryTree:
    ...
    def postorden_super_or_villano(self, is_hero):
        def __postorden_s_v(root, is_hero):
            if root is not None:
                __postorden_s_v(root.right, is_hero)
                if root.other_values is is_hero:
                    print(root.value)
                __postorden_s_v(root.left, is_hero)

        __postorden_s_v(self.root, is_hero)
    ...
    def division_arboles (self, arbol_heroes, arbol_villanos):
        
        def __preorden(root, arbol_heroes, arbol_villanos):
            if root is not None:
                ...
                __preorden(root.left, arbol_heroes, arbol_villanos)
                __preorden(root.right, arbol_heroes, arbol_villanos)
        __preorden (self.root, arbol_heroes, arbol_villanos)
...
#Ejercicio 6 TP
    ...
    def inorden_caracter(self, file_name,caracter):
        def __inorden_caracter(root, file_name,caracter):
            if root is not None:
                __inorden_caracter(root.left, file_name,caracter)
                value = get_value_from_file(file_name, root.other_values)
                if caracter in value[0]:
                    print(root.value, value[0])
                __inorden_caracter(root.right, file_name,caracter)

        __inorden_caracter(self.root, file_name,caracter)

#Ejercicio 23 TP
    def inorden_criatura(self):
        def __inorden_criatura(root):
            if root is not None:
                __inorden_criatura(root.left)
                print(root.value, root.other_values)
                __inorden_criatura(root.right)

        __inorden_criatura(self.root)

    def inorden_add_field_criaturas(self, key, value):
        def __inorden_add_field_criaturas(root, key, value):
            if root is not None:
                __inorden_add_field_criaturas(root.left, key, value)
                root.other_values[key] = value
                __inorden_add_field_criaturas(root.right, key, value)

        __inorden_add_field_criaturas(self.root, key, value)

    def inorden_derrotas(self, murder):
    ...
    def inorden_capture(self, capturador):
        def __inorden(root, capturador):
            if root is not None:
                ...
                if capturador is root.other_values['Capturado']:
                    print(root.value, root.other_values)
